...
const TEST_CASES = [
  ...
  },
  {
    ...
    input: {
      jobUrl:
        'https://www.shopify.com/careers/senior-staff-product-manager-technical_856c1898-01f0-4d01-a31f-3c766e5d5f0b',
  ...
  },
  {
    ...
    input: {
      jobUrl: 'https://jobasaurus--new-ui-p2.deno.dev/job/6289451-product-manager-research',
      additionalContext:
        'Lauren is a data-driven, highly collaborative PM with proven success in driving growth (+250% WAU at InVision) and breaking down complex technical problems (delivering an iterative roadmap to overhaul permissions & sharing at InVision). ',
  ...
  },
  {
    ...
    input: {
      jobUrl:
        'https://jobasaurus--new-ui-p2.deno.dev/job/6332639-senior-product-manager-enterprise',
      additionalContext:
        "Lauren is an action-oriented PM with a strong track record of launching innovative products from the ground up in ambiguous environments (launching some of Freehand's first project planning tools, like Tables: https://www.freehandapp.com/blog/introducing-tables/)",
      resume: Array.from(await Deno.readFile('./intro/fixtures/lauren.pdf')),
    },
  },
  {
    ...
    input: {
      jobUrl:
        'https://jobasaurus--new-ui-p2.deno.dev/job/5907621-senior-product-manager-cx-platform',
      additionalContext:
        "Lauren is an action-oriented PM with a strong track record of launching innovative products from the ground up in ambiguous environments (launching some of Freehand's first project planning tools, like Tables: https://www.freehandapp.com/blog/introducing-tables/)",
      resume: Array.from(await Deno.readFile('./intro/fixtures/lauren.pdf')),
    },
...
async function analyzeQuality(
  ...
  const qualityMetrics: QualityMetrics = {
    ...
    styleMatchesAnalysis:
      (analysis.style === 'DATA_DRIVEN' &&
        additionalContext.toLowerCase().includes('data-driven')) ||
      (analysis.style === 'ACTION_DRIVEN' &&
        additionalContext.toLowerCase().includes('action-oriented')),
  };
...
async function runSingleTest(
  ...
  try {
    ...
    const costs = calculateCosts({
      usage: {
        prompt_tokens:
          Math.ceil(testCase.input.additionalContext.length / 4) +
...
export async function runTests() {
  ...
  const logger = async (message: string) => {
    // Log to file only, remove console output for cleaner tests
  ...
  try {
    ...
    await logger(
      `Average Tokens per Test: ${Math.round((totalCosts.inputTokens + totalCosts.outputTokens) / TEST_CASES.length)}`
    );
